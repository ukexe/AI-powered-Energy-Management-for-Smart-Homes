...
def generate_sample_data(num_buildings):
    days = pd.date_range('1/1/2023', periods=365)
    data = {}
    for n in range(1, num_buildings + 1):
        data[f'Building {n}'] = pd.Series(np.random.randint(50, 100, 365), index=days)
    return data
...
data = generate_sample_data(3)  # Modified to include Building 3
